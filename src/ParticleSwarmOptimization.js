...
export function ParticleSwarmOptimization({
  ...
  const initializePopulation = (populationSize) => {
    ...
    for (let i = 0; i < populationSize; ++i) {
      if (dimension === 2) {
        ...
        newVelocities[i] = [
          gaussianRandom(0, (upperBound - lowerBound) / 100),
          gaussianRandom(0, (upperBound - lowerBound) / 100),
        ];
      }
    }
  ...
  const evolve = () => {
    if (iterations === 0) {
      ...
      let newGlobalBestFitness = evaluate(
        functionEquation,
        newPopulation[0][0],
      );

      for (let i = 0; i < populationSize; ++i) {
        newPersonalBestFitness[i] = evaluate(
          functionEquation,
          newPopulation[0][i],
        );
    ...
    for (let i = 0; i < populationSize; ++i) {
      for (let j = 0; j < dimension - 1; ++j) {
        newVelocities[i][j] =
          ...
          c2 * randomNumberInRange(0, 2) * (globalBest[j] - positions[i][j]);
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="py-2 flex flex-col">
        <div
          className={populationSizeValid ? "text-green-500" : " text-red-500"}
        >
          Population Size:
        </div>
        <input
